...
import { evaluate, fmt, loadBatch } from './lib.mjs';
import { ensureRewrites, expansionFrom } from './rewrites.mjs';
...
async function main() {
  ...
  const cache = await ensureRewrites(batch);
  const opts = { queryExpansion: expansionFrom(cache) };

  let current = { ...DEFAULT_PARAMS };
  const baseline = await evaluate(batch, current, opts);
  ...
  for (let pass = 1; pass <= passes; pass++) {
    ...
    for (const param of ORDER) {
      ...
      for (const value of grid) {
        ...
        const result = await evaluate(batch, trial, opts);
  ...
  if (!process.argv.includes('--no-rerank') && process.env.OLLAMA_API_KEY) {
    ...
    rerankResult = await evaluate(
      batch,
      { ...current, useReranker: true },
      { ...opts, rerank: makeReranker() },
    );
